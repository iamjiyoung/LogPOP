...
%%%  Gloptipoly Code
mpol x 2
p{1} = 0.5*x(1) + 0*x(2) + 0*(1-x(1)-x(2));
p{2} = 0.25*x(1) + 0.75*x(2) + 0.25*(1-x(1)-x(2));
p{3} = 0.25*x(1) + 0.25*x(2) + 0.5*(1-x(1)-x(2));
p{4} = 0*x(1) + 0*x(2) + 0.25*(1-x(1)-x(2));
...
ops.mosek.MSK_DPAR_INTPNT_CO_TOL_PFEAS = 1e-9;
ops.mosek.MSK_DPAR_INTPNT_CO_TOL_DFEAS = 1e-9;
ops.mosek.MSK_DPAR_INTPNT_CO_TOL_REL_GAP = 1e-9;

optimize(MomRelax,obj,ops); % Unreported warm-up solve
started = tic;
sol = optimize(MomRelax,obj,ops);
elapsed = toc(started)
