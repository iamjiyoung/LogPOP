...
for row = 1:num_cases
    ...
    instance = Table_5_generate_instance(n(row), m(row), seed(row), config.epsilon);
    local = Table_5_solve_sqp(instance, config.num_starts);
    standard = Table_5_solve_log_relaxation(instance, false, 1);
    ...
    elseif lme_psd_dim(row) > config.max_lme_psd_dim
        ...
        try
            tighter = Table_5_solve_log_relaxation(instance, true, lme_order(row));
    ...
    elseif full_sos_psd_dim(row) > config.max_full_sos_psd_dim
        ...
        try
            full_sos = Table_5_solve_direct_product(instance, full_sos_order(row), ...
