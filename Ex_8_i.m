...
At_x1 = NN(1)*log(pp11) + NN(2)*log(pp12);

pp21 = double(subs(p{1},x,xx{2}));
pp22 = double(subs(p{2},x,xx{2}));

At_x2 = NN(1)*log(pp21) + NN(2)*log(pp22);

fprintf('\n=== Example 8(i): reported result ===\n');
...
fprintf('Extracted optimizers and objective values:\n');
fprintf('  atom 1: %s, value = %.10f\n', mat2str(xx{1}(:)', 10), At_x1);
fprintf('  atom 2: %s, value = %.10f\n', mat2str(xx{2}(:)', 10), At_x2);
