% Reproduce the scaling results reported in Table 2.
%
% J. Choi, July 31, 2026
...
for row = 1:num_cases
    ...
    instance = Table_2_generate_instance(n(row), m(row), seed(row), config.epsilon);
    local = Table_2_solve_sqp(instance, config.num_starts);
    standard = Table_2_solve_log_relaxation(instance, false, 1);
    ...
    elseif lme_psd_dim(row) > config.max_lme_psd_dim
        ...
        try
            tighter = Table_2_solve_log_relaxation(instance, true, lme_order(row));
    ...
    elseif full_sos_psd_dim(row) > config.max_full_sos_psd_dim
        ...
        try
            full_sos = Table_2_solve_direct_product(instance, full_sos_order(row), ...
...
fprintf('\n=== Table 2: quantities reported in the paper ===\n');
paper_results = results(:, {'study','n','m','log_gap', ...
    'mom_time','mom_psd_dim','lme_psd_dim','full_sos_psd_dim'});
disp(paper_results)

fprintf('\n=== Table 2: LME quantities reported in the text ===\n');
lme_report = results(results.study == "m" & ismember(results.m, [2 4]), ...
    {'m','lme_log_gap','lme_time'});
disp(lme_report)

fprintf('\n=== Table 2: direct-product quantities reported in the text ===\n');
