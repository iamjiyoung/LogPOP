% Reproduce the scaling results reported in Table 7.
%
% J. Choi, July 31, 2026
...
for row = 1:num_cases
    ...
    instance = Table_7_10_generate_instance(n(row), m(row), seed(row), config.epsilon);
    local = Table_7_10_solve_sqp(instance, config.num_starts);
    standard = Table_7_10_solve_log_relaxation(instance, false, 1);
    ...
    elseif lme_psd_dim(row) > config.max_lme_psd_dim
        ...
        try
            tighter = Table_7_10_solve_log_relaxation(instance, true, lme_order(row));
    ...
    elseif full_sos_psd_dim(row) > config.max_full_sos_psd_dim
        ...
        try
            full_sos = Table_7_10_solve_direct_product(instance, full_sos_order(row), ...
